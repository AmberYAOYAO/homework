...
#         'LOCATION':"",#变量，缓存的名称，文件缓存，就是文件名，数据库缓存就是数据库名
#         'TIMEOUT':300,#缓存时间，默认300秒，如果为none代表永不过期
#         'OPTIONS':{
...
#         'LOCATION':"os.path.join(BASE_DIR,"cache_file")",#变量，缓存的名称，文件缓存，就是文件名，数据库缓存就是数据库名
#         'TIMEOUT':300,#缓存时间，默认300秒，如果为none代表永不过期
#         'OPTIONS':{
#             'MAX_ENTRIES':300,#最大缓存个数，默认也是300个
#             'CULL_FREQUENCY': 3 #到达最大缓存数之后，删除缓存的比例，默认是3
...
#     }
# }
#DATABASE_ROUTERS = ["Qshop.myrouter.Router"]