...
def goods(request,id):
    ...
    email = request.COOKIES.get("email")
    if request.method == "POST":
        count_num = request.POST.get("count_num")
        number = int(count_num)
        #保存订单主表
        p_order = Pay_order()
        p_order.order_id = str(time.time()).replace(".","")
...
def user_order(request):
    email = request.COOKIES.get("email")#获取邮件
    user = Quser.objects.filter(email=email).first() #获取对应用户
    if user:
        order_list = user.pay_order_set.all() #获取对应订单
...
def get_address(request):
    addr = GoodsAddress()
    addr.receiver = "tom"
    addr.address = "fancy house"
    addr.postcode= "1111"
    addr.phone = "911"
    addr.state = '1'
    # addr.user = Quser.objects.using("slave").get(id=1)
    # addr.save(using="default")
    addr.user = Quser.objects.get(id=1)
    addr.save()
    return HttpResponse("马什么梅")